...
Feature: Lesen der Ressource Observation (@Observation-Read-Herzfrequenz)
  ...
  Scenario: Read einer Observation anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath 'id.replaceMatches("/_history/.+","").matches("${data.observation-read-herzfrequenz-id}")' with error message 'ID der Ressource entspricht nicht der angeforderten ID'
    ...
    And FHIR current response body is a valid ISIK3 resource and conforms to profile "https://gematik.de/fhir/isik/v3/VitalparameterUndKoerpermasze/StructureDefinition/ISiKHerzfrequenz"
