...
Feature: Lesen der Ressource Encounter (@Encounter-Read-In-Progress)
  ...
  Scenario: Vorbedingung
    ...
    """
      ...
      Optional - Leistungsanbieter (Referenz): Beliebig (Bitte ID im shared.yaml eingeben)
      Optional - Ort: Beliebig (Bitte ID im shared.yaml eingeben)
    """
  ...
  Scenario: Read eines Encounter anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath 'id.replaceMatches("/_history/.+","").matches("${data.encounter-read-in-progress-id}")' with error message 'ID der Ressource entspricht nicht der angeforderten ID'
