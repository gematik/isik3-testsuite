...
Feature: Testen von Suchparametern gegen die Communication Ressource (@Communication-Search)
  ...
  Scenario Outline: Suche nach der Nachricht anhand des <title>
    ...
    Examples:
      ...
      | Patienten  | xml         | subject         | subject   | Patient/      | ${data.terminplanung-patient-id}      |
      | Patienten  | xml         | patient         | subject   |               | ${data.terminplanung-patient-id}      |
      | Empfängers | json        | recipient       | recipient | Practitioner/ | ${data.terminplanung-practitioner-id} |
      | Senders    | json        | sender          | sender    | Patient/      | ${data.terminplanung-patient-id}      |
