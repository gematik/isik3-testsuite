...
Feature: Lesen der Ressource DocumentReference (@DocumentReference-Read)
  ...
  Scenario: Vorbedingung
    ...
    """
      ...
      - die ID der korrespondierenden FHIR-Ressource zu diesem Testdatensatz muss in der dokumentenaustausch.yaml eingegeben worden sein.
      ...
      Patientenbezug: Beliebig (Bitte ID im dokumentenaustausch.yaml eingeben)
      ...
      Kontext: Pathologiebesuch im Rahmen eines Krankenhauskontakts
    """

  Scenario: Read und Validierung des CapabilityStatements
    Then Get FHIR resource at "http://fhirserver/metadata" with content type "json"
    And FHIR current response body evaluates the FHIRPath 'rest.where(mode = "server").resource.where(type = "DocumentReference" and interaction.where(code = "read").exists()).exists()'
  ...
  Scenario: Read von Dokumentenmetadaten anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath 'id.replaceMatches("/_history/.+","").matches("${data.documentreference-read-id}")' with error message 'ID der Ressource entspricht nicht der angeforderten ID'
    And FHIR current response body is a valid CORE resource and conforms to profile "http://hl7.org/fhir/StructureDefinition/DocumentReference"
    And FHIR current response body is a valid ISIK3 resource and conforms to profile "https://gematik.de/fhir/isik/v3/Dokumentenaustausch/StructureDefinition/ISiKDokumentenMetadaten"
    ...
    And FHIR current response body evaluates the FHIRPath "type.coding.where(system = 'http://ihe-d.de/CodeSystem/IHEXDStypeCode' and code = 'PATH' and display = 'Pathologiebefundberichte').exists()" with error message 'Der Dokumententyp (XDS) Code entspricht nicht dem Erwartungswert'
    And FHIR current response body evaluates the FHIRPath "subject.reference.replaceMatches('/_history/.+','').matches('${data.patient-read-id}')" with error message 'Der Patientenbezug entspricht nicht dem Erwartungswert'
