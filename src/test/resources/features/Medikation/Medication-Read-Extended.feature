...
Feature: Lesen der Ressource Medication (@Medication-Read-Extended)
  ...
  Scenario: Read eines Medikaments anhand der ID
    ...
    And FHIR current response body evaluates the FHIRPath 'id.replaceMatches("/_history/.+","").matches("${data.medication-read-extended-id}")' with error message 'ID der Ressource entspricht nicht der angeforderten ID'
